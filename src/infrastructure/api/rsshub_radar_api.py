"""RSSHub Radar API 客户端

提供 RSSHub 路由发现、搜索和订阅 URL 构建功能。
"""
...
def normalize_base_url(base_url: str) -> str:
    """将 base URL 规范化为 scheme + netloc 形式。"""
...
class RSSHubRadarAPI:
    ...
    def build_subscribe_url(
        ...
        *,
        ...
        """从 base URL + uri + 查询参数构建完整的订阅 URL。"""
        resolved_base_url = self._resolve_base_url(explicit_base_url, default_base_url)
        normalized_uri = normalize_uri(uri)
    ...
    async def _get_rules(self, resolved_base_url: str) -> list[dict[str, Any]]:
        """从 /api/radar/rules 加载并规范化路由规则，带缓存。"""
        now = time.monotonic()
